...
export default function FriendRequestsList({
  ...
  useEffect(() => {
    const getPendingFriendsReceived = async () => {
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/user/${userID}/getPendingFriendsReceived`,
        {
          method: "GET",
          headers: {
  ...
  const acceptFriendRequest = async (friendID) => {
    try {
      const userID = localStorage.getItem("userID");
      const response = await fetch(
        `http://localhost:3000/api/v1/user/${userID}/friend-request/accept`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (data.success) {
        ...
        setPendingFriendsReceived(
          pendingFriendsReceived.filter((friend) => friend._id !== friendID)
        );
      }
    } catch (err) {
