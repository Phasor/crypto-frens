...
export default function FriendRequestsList({
    ...
    useEffect(() => {
        const getPendingFriendsReceived = async () => {
            ...
            const response = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/user/${userID}/getPendingFriendsReceived`,
                {
                    method: "GET",
                    headers: {
    ...
    const acceptFriendRequest = async (friendID) => {
        try {
            const userID = localStorage.getItem("userID");
            const response = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/user/${userID}/friend-request/accept`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
