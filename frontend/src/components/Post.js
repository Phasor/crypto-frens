...
export default function Post({ setPosts, post }) {
    ...
    const postComment = async (e) => {
        ...
        try {
            const response = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/post/${postID}/comment`,
                {
                    method: "POST",
                    headers: {
    ...
    const likePost = async (e) => {
        try {
            const response = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/post/${post._id}/like`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const unlikePost = async (e) => {
        try {
            const response = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/post/${post._id}/unlike`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                {post.imgURL && !showModal && (
                    <div className="h-56 md:h-96 bg-white">
                        <img
                            src={post.imgURL}
                            alt=""
                            className="h-full w-full object-cover max-h-96 cursor-pointer"
                ...
                <div className="flex justify-start space-x-4 mt-1 p-1 w-100%">
                    {!postLiked ? (
                        <>
                            <HeartIconOutline
                                onClick={likePost}
                                className="h-6 w-6 cursor-pointer transform hover:scale-110"
                            />
                            <ChatBubbleOvalLeftIcon
                        ...
                        <>
                            <HeartIconSolid
                                onClick={unlikePost}
                                className="h-6 w-6 cursor-pointer text-red-500 transform hover:scale-110"
                            />
                            <ChatBubbleOvalLeftIcon
                                className="h-6 w-6 transform hover:scale-110"
                                onClick={() => setShowComments(!showComments)}
                        ...
                        <div>
                            <form onSubmit={postComment}>
                                <input
                                    ...
                                    className="w-full bg-gray-100 border rounded-md p-1 mt-4 outline-none text-gray-600"
                                    type="text"
                                    name="comment"
                                    id="comment"
                                    value={comment}
