        ...
        if (user && friend) {
            ...
            if (user.friends.includes(friendObjID)) {
                ...
                await User.updateOne(
                    { _id: userObjID },
                    { $pull: { pendingFriendRequestsReceived: friendObjID } }
                );
                await User.updateOne(
                    { _id: friendObjID },
                    { $pull: { pendingFriendRequestsSent: userObjID } }
                );
...
exports.getPendingFriendsReceived = async (userID) => {
    try {
        const pendingFriendIDs = await User.findById(userID).select(
            "pendingFriendRequestsReceived"
        );
        const pendingFriends = [];
        for (
            let i = 0;
            i < pendingFriendIDs.pendingFriendRequestsReceived.length;
            i++
        ) {
            const pendingFriendDetails = await User.find({
                _id: pendingFriendIDs.pendingFriendRequestsReceived[i],
...
exports.getPendingFriendsSent = async (userID) => {
    try {
        ...
        for (
            let i = 0;
            i < pendingFriendIDs.pendingFriendRequestsSent.length;
            i++
        ) {
            const pendingFriendDetails = await User.find({
                _id: pendingFriendIDs.pendingFriendRequestsSent[i],
            }).select("firstName lastName username _id profileImage");
            pendingFriends.push(pendingFriendDetails[0]);
        }
        return pendingFriends;
...
exports.getAllUsersExFriends = async (userID) => {
    try {
        ...
        const pendingFriendRequestsReceived =
            user.pendingFriendRequestsReceived;
        const allUsers = await User.find();
        const allUsersExFriends = allUsers.filter((user) => {
            return (
                !friends.includes(user._id) &&
                !pendingFriendRequestsSent.includes(user._id) &&
                !pendingFriendRequestsReceived.includes(user._id) &&
                user._id != userID
            );
        });
