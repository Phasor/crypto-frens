    ...
    describe("given we signup a new user", () => {
        it("should return the new user details", async () => {
            const response = await request(app)
                ...
                .then((response) => {
                    expect(200);
                    expect(response.body.user.user.username).toEqual(
                        "a@test.com"
                    );
        ...
        it("should return the new user details", async () => {
            const response = await request(app)
                ...
                .then((response) => {
                    ...
                    expect(response.body.user.user.shortName).toEqual(
                        "bSmythe"
                    );
        ...
        it("friend requests should be reflected in both users", async () => {
            const response = await request(app)
                ...
                .then((response) => {
                    expect(200);
                    expect(
                        response.body.user.pendingFriendRequestsSent
                    ).toContain(User2ID);
                    expect(
                        response.body.friend.pendingFriendRequestsReceived
                    ).toContain(User1ID);
                });
        });
    });
    ...
    describe("given user 2 comments on post 1", () => {
        it("post should have 1 comment from user 2", async () => {
            const response = await request(app)
                ...
                .then((response) => {
                    expect(200);
                    expect(response.body.post.comments[0].author).toEqual(
                        User2ID
                    );
                    expect(response.body.post.comments[0].comment).toEqual(
                        "This is a comment"
                    );
